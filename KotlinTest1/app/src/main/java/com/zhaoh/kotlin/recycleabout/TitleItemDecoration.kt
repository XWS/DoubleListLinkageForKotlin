 ...
 */
class TitleItemDecoration(context: Context, mData: List<CityBean>) : RecyclerView.ItemDecoration() {

    var mTitleHeight: Int
    var mTitleSize: Int
    var mPaint: Paint? = null
    var mData: List<CityBean>? = null
    ...
    override fun onDraw(c: Canvas?, parent: RecyclerView?, state: RecyclerView.State?) {
        ...
        while (index < childCount) {
            ...
            if (positon > -1) {
                if (positon == 0) {
                    drawTitleArea(c, left!!, right, childView, layoutParams, positon)
                } else {
                    if (mData!![positon - 1].tag != mData!![positon].tag) {
                        drawTitleArea(c, left!!, right, childView, layoutParams, positon)
                    } else {
    ...
    override fun onDrawOver(c: Canvas?, parent: RecyclerView?, state: RecyclerView.State?) {
        ...
        val childView = parent.findViewHolderForLayoutPosition(postion).itemView
